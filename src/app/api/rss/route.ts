...
function generateRSSFeed(posts: any[]): string {
	...
	// 过滤已发布的文章并限制数量
	const publishedPosts = posts.filter(post => post.status === "publish").slice(0, 20); // 限制最新20篇文章
    ...
    <author>${baseSEOConfig.author.email} (${baseSEOConfig.author.name})</author>
${categories}
  </item>`;
		})
		.join("\n");
...
<rss version="2.0" 
  ...
  <channel>
    ...
    <managingEditor>${baseSEOConfig.author.email} (${baseSEOConfig.author.name})</managingEditor>
    <webMaster>${baseSEOConfig.author.email} (${baseSEOConfig.author.name})</webMaster>
...
export async function POST(request: NextRequest) {
	try {
		...
		return NextResponse.json({ error: "Failed to refresh RSS cache" }, { status: 500 });
	}
}
