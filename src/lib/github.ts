...
export const sanitizeRepositoryData = (
...
export const calculateTotalStats = (repositories: GitHubRepository[]) => {
  ...
  return {
    ...
    totalStars: validRepos.reduce(
      (sum, repo) => sum + (repo.stargazers_count || 0),
      0
    ),
    totalForks: validRepos.reduce(
      (sum, repo) => sum + (repo.forks_count || 0),
      0
    ),
    totalWatchers: validRepos.reduce(
...
export const getRepositoryActivityLevel = (
  repo: GitHubRepository
): "high" | "medium" | "low" => {
  const now = new Date();
  const updatedAt = new Date(repo.updated_at);
...
export const filterRepositories = (
  ...
  return repositories.filter((repo) => {
    ...
    if (
      ...
      (repo.stargazers_count || 0) < filters.minStars
    ) {
      return false;
    }
    if (
      filters.maxStars !== undefined &&
      (repo.stargazers_count || 0) > filters.maxStars
    ) {
      return false;
    }
