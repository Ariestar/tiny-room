...
export function createResponsiveAnimationConfig(config: {
  ...
  return {
    duration: isReducedMotion
      ? config.reducedMotionDuration ?? 0.1
      : config.duration ?? 1,
...
export class AnimationPerformanceMonitor {
  ...
  startAnimation(type: string = "unknown") {
    this.animationCount++;
    ...
    if (this.animationCount > this.maxAnimations) {
      console.warn(
        `Animation count exceeded ${this.maxAnimations}. Consider optimizing.`
      );
    }
  ...
  getPerformanceMetrics() {
    return {
      ...
      totalAnimations: this.performanceMetrics.length,
      averageDuration:
        this.performanceMetrics
          .filter((m) => m.duration)
          .reduce((sum, m) => sum + (m.duration || 0), 0) /
          this.performanceMetrics.filter((m) => m.duration).length || 0,
      metrics: this.performanceMetrics.slice(-50), // 保留最近50个
    };
  }
...
// 导出便捷访问
export const { variants, springs, easings, parallax, performanceMonitor } =
  animationUtils;
