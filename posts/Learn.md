...
## 2025-01-14: 修复 TOC 目录和进度条显示问题

修复了两个关键的用户界面问题：TOC 目录不显示 H1 标签和进度条位置优化。

### 修复的问题

1. **TOC 目录 H1 标签不显示**：rehype-extract-toc 插件只提取 h2-h6 标签，遗漏了 h1
2. **进度条位置**：顶部进度条可能与 Header 冲突，改为侧边更美观

### 解决方案

1. **TOC 修复**：

    - 更新`rehype-extract-toc.ts`包含 h1 标签
    - 重构 padding 计算逻辑，使用`minDepth`作为基准避免负数
    - 增强视觉效果：活跃项添加左边框和加粗样式
    - 改进文字：将"On this page"改为中文"目录"

2. **进度条优化**：
    - 改为垂直侧边进度条，位置`fixed right-4 top-1/2`
    - 添加百分比数字显示，方便用户了解阅读进度
    - 使用`translate-y-1/2`精确居中定位
    - 渐变方向从`gradient-to-r`改为`gradient-to-b`

### 技术细节

-   TOC 层级计算：`relativeDepth = item.depth - minDepth`确保最小缩进为 0
-   进度条尺寸：`w-1 h-64`创建细长的垂直条
-   活跃状态：`border-l-2 border-blue-500`提供清晰的视觉反馈
-   响应式设计：两个组件都完全支持深浅主题切换
